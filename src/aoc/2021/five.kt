...
fun main() {
    ...
    lines.forEach { (start, finish) ->
        ...
        if (x1 == x2) {
            ...
            (yIndices).map { Pair(x1, it) }.forEach { visit -> visitedCoords.putOrIncrease(visit) }
        } else if (y1 == y2) {
            val xIndices = if (x1 > x2) x2..x1 else x1..x2
            (xIndices).map { Pair(it, y1) }.forEach { visit -> visitedCoords.putOrIncrease(visit) }
        } else {
            var x = x1
            var y = y1
            while (true) {
...
fun MutableMap<Pair<Int, Int>, Int>.putOrIncrease(coord: Pair<Int, Int>) {
    this[coord] = getOrDefault(coord, 0) + 1
}
